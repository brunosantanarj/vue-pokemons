...
async function listAllDetailed({ results }: APIPaginatedResponse<Pokemon[]>) {
  const avaiableRequests = await Promise
    .all(results.map(pokemon => fetch(`${endpoint}/${extractID(pokemon.url)}`)))
  return await Promise.all(avaiableRequests.map(request => request.json()))
}

export async function listAll({ pageParam = '0' }): Promise<APIPaginatedResponse<PokemonExplained[]>> {
